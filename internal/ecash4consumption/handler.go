...
func parseFilters(q url.Values) (FilterParams, error) {
	...
	return FilterParams{
		...
		TariffClass: httpx.CSV(q, "tariff"),
		MeterSerial: httpx.CSV(q, "meterSerial"),
		Search:      q.Get("search"),
		DateFrom:    from,
		DateTo:      to,
	}, nil
...
func (h *Handler) Detail(w http.ResponseWriter, r *http.Request) {
	...
	if err != nil {
		h.log.Error("ecash4 consumption detail failed", zap.Error(err))
...
func (h *Handler) Aggregate(w http.ResponseWriter, r *http.Request) {
	...
	if err != nil {
		h.log.Error("ecash4 consumption aggregate failed", zap.Error(err))
		httpx.Error(w, http.StatusInternalServerError, "internal error")
		return
	}
	httpx.JSON(w, http.StatusOK, result)
